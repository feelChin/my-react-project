...
function TheQuest() {
  ...
  const [state, setState] = useState(fill(3));
  ...
  const listenerNode = useCallback(() => {
    ...
    element.forEach((item) => {
      const { left, top, width, zIndex } = item.getBoundingClientRect();
      elementArray.push({
        left,
        top,
        width,
    ...
    elementArray.forEach((item, index) => {
      ...
      elementArray.forEach((itemChild, itemChildIndex) => {
  ...
  function renderSpace() {
    ...
    return spaceList.map((item, index) => {
      let topLevel = 0;
  ...
  useEffect(() => {
    ...
    function settlementUnique() {
      ...
      if (findRepeat) {
        itemAniAll.forEach((element, index) => {
